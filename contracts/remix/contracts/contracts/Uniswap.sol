...
contract Uniswap {
    string[] public tokens = ['falopaCoinA', 'falopaCoinB', 'falopaCoinC'];
    ...
    function swapTokenToEth(string memory tokenName, uint _amount) public returns (uint) { //payable no estaba originalmente
        uint exactAmount = _amount / 10 ** 18;
        uint ethToBeTransferred = exactAmount * ethValue;
        
        require(address(this).balance >= ethToBeTransferred, "Low balance");
